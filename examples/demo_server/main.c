/* Demo MCP server built on the SDK, exposing three message-returning tools:
 *   - "greet"    : returns a friendly greeting (uses `name` arg, defaults to "world")
 *   - "get_time" : returns the current UTC time as an ISO-8601 string
 *   - "motd"     : returns a static message-of-the-day
 */
...
static int greet_tool(const cJSON *args, mcp_tool_response_t *resp, void *userdata)
{
    ...
    char msg[256];
    int written = snprintf(msg, sizeof msg, "Hello, %s! Welcome to the C23 MCP SDK.", name);
...
int main(void)
{
    mcp_server_t *srv = mcp_server_create("demo-server", "0.1.0");
    if (srv == NULL) {
